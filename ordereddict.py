# -*- coding: utf-8 -*-
"""
Created on Thu Feb 27 17:51:42 2020
...
if __name__ == "__main__":
    ...
    for i in range(N):
        lis = input().split(" ")
        if(len(lis)==3):
            item_name=lis[0] + " " + lis[1]
            price=int(lis[2])
        else:
            item_name=lis[0]
            price=int(lis[1])

        if item_name not in orderdic:
            orderdic[item_name] = price
        else:
            orderdic[item_name] = orderdic[item_name] + price
